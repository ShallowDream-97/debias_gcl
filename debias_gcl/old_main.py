...
model = LightGCL(adj_norm.shape[0], adj_norm.shape[1], d, u_mul_s, v_mul_s, svd_u.T, svd_v.T, train_csr, adj_norm, l, temp, lambda_1, dropout, batch_user, args.alpha,device)
...
for epoch in range(epoch_no):
    ...
    for batch in tqdm(range(batch_no)):
        ...
        # feed
        optimizer.zero_grad()
        loss, loss_r, loss_s= model(uids, iids, pos, neg,active_list = active_list,eps_1 = args.eps_1,eps_2 = args.eps_2,eps_3 = args.eps_3,loss_flag = args.loss_flag)
        loss.backward()
        optimizer.step()
        #print('batch',batch)
    ...
    if epoch % 3 == 0:  # test every 10 epochs
        ...
        for batch in tqdm(range(batch_no)):
            ...
            predictions = model(test_uids_input,None,None,None,active_list = active_list,eps_1 = args.eps_1,eps_2 = args.eps_2,eps_3 = args.eps_3,loss_flag = args.loss_flag,test=True)
...
metric.to_csv('log/result_'+args.data+'_'+time.strftime('%Y-%m-%d_%H_%M_%S',current_t)+'_'+str(random_seed)+'_'+str(args.eps_1)+'_'+str(args.eps_2)+'_'+str(args.eps_3)+'_'+str(args.active_threshold)+'_'+str(args.unactive_threshold)+'.csv')
